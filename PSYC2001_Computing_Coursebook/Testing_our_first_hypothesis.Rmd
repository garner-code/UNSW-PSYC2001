...
We do this by adding  inside `ggplot()`. The `aes()` function is where we tell `ggplot()` what variables to use for the x and y axes, as well as other aesthetic features like colour and shape.
...
### Activity - So you've visualised your data, now what?
...
Now there is one more argument that we super care about setting when performing a t test. This is the `paired` argument. The `paired` argument tells R whether the two sets of data we are comparing are from the same people (i.e. paired) or from different people (i.e. unpaired). Here, we know that the good and bad mood likes are from the same people, so we need to set `paired = TRUE`. If we were comparing likes from two different groups of people, we would set `paired = FALSE`.
...
t.test(x=social_media_diff$likes_diff) #the formula likes_diff~1 indicates that there is no grouping variables (i.e just compare to 0!)
```
