...
Add some info about wideform and longform - e.g. webpage.
...
social_media_likes <- social_media %>% 
  ...
  pivot_longer(cols = ends_with("likes"), names_to = "mood", values_to = "likes") # add a comment here
```

Add a head(dataframe)
...
Activity here where they have to replicate the code but pick a different theme.

------------------------------------------------------------------------

### Testing hypotheses manually

Now that we have had a look at the structure of our data we have have a go at using a t-test to compare the number of likes in a good and bad mood. We could of course do this manually, by hand (as we have in the tutorials). We have done this below using the first 10 values for good and bad mood likes, to remind you of allll the steps involved.
...
We can very easily calculate a t-test in R using the `t.test()` function. Please first have a look at this function using the `?` to understand its arguments. While we usually put a code block here to show you how to do this, we recommend from now on that you do this yourself instead, using the console. You are ready. Fly my pretty!
...
Now that we have conducted our statistical test we need to present the results in the simplest way possible. Sometimes the best way to plot the data to get the message across is not the same as the one you initially used to visualize and learn about the data. One way to visualize mean differences between conditions is by getting the average number of likes when participants are in a good and bad mood and plotting it using a column graph.
