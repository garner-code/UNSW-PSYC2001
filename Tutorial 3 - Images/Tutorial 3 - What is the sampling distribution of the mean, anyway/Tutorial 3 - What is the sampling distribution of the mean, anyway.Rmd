...
global_social_media <- read.csv(file=here("Data", #fill this in) ) # an example for previous tutorials
...
Can you find the other university [add uni code here, from readme]?
...
```{r}
set.seed(1) #ensures we always get the same result from sampling ! ## put a link to a webpage about it
...
Now, what we want to do next is to visualise this data using a histogram. However, there is a problem.  The `sample()` function provides us with a single vector (column) but our plotting function `ggplot()` only likes dataframes. So we first need to convert our `mean_sample_20` vector into a dataframe. 

To do this we use the `data.frame()` function which is used to create dataframes in R. 
...
#fill in the code below ! 
mean_sample_100_df <- data.frame(sample_100 = mean_sample_100,
 sample_250 = mean_sample_250) #create a dataframe with a column called sample_250 that takes values from our vector 
#etc
